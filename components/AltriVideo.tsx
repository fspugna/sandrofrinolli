...
export default async function AltriVideo({ currentId }: { currentId: string }) {
    ...
    return (
        <section className="mt-24 border-t border-white/10 pt-16">
            <div className="flex justify-between items-end mb-10">
                ...
                <Link href="/video" className="text-blue-400 hover:text-blue-300 uppercase tracking-widest text-xs transition-colors">
                    Vedi tutti i video →
                </Link>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8">
                {altriVideo.map((v: Video) => {
                    ...
                    return (
                        <Link key={v._id} href={`/video/${v._id}`} className="group block bg-[#272833] rounded-lg overflow-hidden">
